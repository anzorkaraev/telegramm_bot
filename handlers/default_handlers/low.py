...
from states.low_price import LowPrice
...
def search_ticket(message: Message) -> None:
    ...
    :return:
    """
    bot.set_state(message.from_user.id, LowPrice.origin, message.chat.id)
    bot.send_message(message.chat.id, f'Город отправления')


@bot.message_handler(state=LowPrice.origin)
def get_origin(message: Message) -> None:
    ...
    if iata_city_code == 'error':
        ...
        bot.set_state(message.from_user.id, LowPrice.destination, message.chat.id)
...
@bot.message_handler(state=LowPrice.destination)
def get_destination(message: Message) -> None:
    ...
    if iata_city_code == 'error':
        ...
        bot.set_state(message.from_user.id, LowPrice.depart_date, message.chat.id)
...
@bot.message_handler(state=LowPrice.depart_date)
    ...
    try:
        ...
        if Date.is_date_valid(dep_date):
            ...
            bot.send_message(message.chat.id, f'Если вам нужен билет в обратную сторону, '
                                              ...
                                              f'\n'
                                              f'Сегодня {today.strftime("%d.%m.%Y")}', parse_mode='html')
            bot.set_state(message.from_user.id, LowPrice.return_date, message.chat.id)
...
@bot.message_handler(state=LowPrice.return_date)
