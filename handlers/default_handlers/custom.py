...
from states.custom_price import CustomPrice
...
def search_ticket(message: Message) -> None:
    ...
    :return:
    """
    bot.set_state(message.from_user.id, CustomPrice.origin, message.chat.id)
    bot.send_message(message.chat.id, f'Город отправления')


@bot.message_handler(state=CustomPrice.origin)
...
@bot.message_handler(state=CustomPrice.destination)
def get_destination(message: Message) -> None:
    ...
    if iata_city_code == 'error':
        ...
        bot.send_message(message.chat.id, 'Укажите ценовой диапазон (от - до)\n'
                                          '<b>Через пробел!!!</b>', parse_mode='html')
...
@bot.message_handler(state=CustomPrice.price_range)
def get_price_range(message: Message):
    try:
        start_price, end_price = message.text.split()
        if start_price.isdigit() and end_price.isdigit():
            today = date.today()
            bot.send_message(message.chat.id, f'Введите дату отправления в формате <b>ДД ММ</b>'
                                              f' либо <b>ДД ММ ГГГГ</b>\n'
                                              f'Через пробел!\n'
                                              f'\n'
                                              f'Сегодня {today.strftime("%d.%m.%Y")}', parse_mode='html')
            bot.set_state(message.from_user.id, CustomPrice.depart_date, message.chat.id)
...
@bot.message_handler(state=CustomPrice.depart_date)
    ...
    try:
        ...
        if Date.is_date_valid(dep_date):
            ...
            bot.send_message(message.chat.id, f'Если вам нужен билет в обратную сторону, '
                                              ...
                                              f'\n'
                                              f'Сегодня {today.strftime("%d.%m.%Y")}', parse_mode='html')
            bot.set_state(message.from_user.id, CustomPrice.return_date, message.chat.id)
...
@bot.message_handler(state=CustomPrice.return_date)
def get_return_date(message: Message):
    if message.text.lower() == 'нет':
        ...
        tickets_in_specified_range = [
            ticket for ticket in ticket_price['data']
            if ticket['price'] in range(int(data['start_price']), int(data['end_price']) + 1)
        ]

        if ticket_price['success'] is not True or len(ticket_price['data']) == 0\
                or len(tickets_in_specified_range) == 0:
            bot.send_message(message.chat.id, '❎ По вашему запросу нет данных.\n'
                                              'Попробуйте ещё раз')
            bot.delete_state(message.from_user.id, message.chat.id)
        else:
            count = 1
            for ind in range(len(tickets_in_specified_range)):
                dep_date_text = datetime.strptime(
                    tickets_in_specified_range[ind]["departure_at"], '%Y-%m-%dT%H:%M:%S%z'
                )
                ...
                text = (
                    ...
                    f'Цена 📈 : <b>{tickets_in_specified_range[ind]["price"]} {ticket_price["currency"]}</b>\n'
                    f'Номер рейса #️⃣ : {tickets_in_specified_range[ind]["flight_number"]}\n'
                    f'Кол-во остановок на пути ➡️⛔️: {tickets_in_specified_range[ind]["transfers"]}\n'
                )
                count += 1

                bot.send_message(
                    message.chat.id, text,
                    reply_markup=weather(tickets_in_specified_range[ind]['link'], data['destination']),
                    parse_mode='html'
                )
            bot.send_message(
                message.chat.id,
                ...
                if ticket_price['success'] is not True or len(ticket_price['data']) == 0\
                    ...
                    for ind in range(len(tickets_in_specified_range)):
                        ...
                        ret_date_text = datetime.strptime(
                            tickets_in_specified_range[ind]["return_at"], '%Y-%m-%dT%H:%M:%S%z'
                        )
                        ...
                        text = (
                            ...
                            f'Номер рейса #️⃣ : {tickets_in_specified_range[ind]["flight_number"]}\n'
                            f'Кол-во остановок на пути ➡️⛔️: {tickets_in_specified_range[ind]["transfers"]}\n'
                            f'Кол-во остановок на обратном пути ⛔️⬅️: '
                            f'{tickets_in_specified_range[ind]["return_transfers"]}'
                        )
                        count += 1
