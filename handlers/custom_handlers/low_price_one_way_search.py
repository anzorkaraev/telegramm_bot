...
from states.low_price_one_way import LowPriceOneWay
from telebot.types import Message

from utils.checking_the_date import Date


@bot.message_handler(func=lambda message: message.text == 'В одну сторону ➡️')
def search_ticket(message: Message) -> None:
    ...
    :return:
    """
    bot.set_state(message.from_user.id, LowPriceOneWay.origin, message.chat.id)
    bot.send_message(message.chat.id, f'Город отправления')


@bot.message_handler(state=LowPriceOneWay.origin)
def get_origin(message: Message) -> None:
    ...
    if len(iata_city_code) == 0:
        ...
        bot.set_state(message.from_user.id, LowPriceOneWay.destination, message.chat.id)
...
@bot.message_handler(state=LowPriceOneWay.destination)
def get_destination(message: Message) -> None:
    ...
    if len(iata_city_code) == 0:
        ...
        bot.set_state(message.from_user.id, LowPriceOneWay.depart_date, message.chat.id)
...
@bot.message_handler(state=LowPriceOneWay.depart_date)
